...
# jaxtyping's symbolic shape strings (for example ``"bdim obs_size"``) are
# valid jaxtyping syntax but not valid Python expressions for newer typeguard
...
from dva_quadrotor_mjx.algorithms.shac.trainer_utils import fjac_env_step, fscannable_jac_env_step, fjac_rollout, fload_checkpoint, frender_states, fget_image
...
class SHAC:
    ...
    def train(self):
        ...
        for it in range(self.num_evals_after_init):
            ...
            save_to = str(Path(self.checkpoint_dir, Path(file_name)))
